        ...
        function ($rootScope, $filter, $modal, $alert, $translate, FormResource, RuntimeResource, AtmosphereService, ToolManager, User) {
            ...
            $rootScope.$on('runtime.process.change-state', function (event, message) {
                if (message.arguments.state === states.completed) {
                    RuntimeResource.readProcess({key: message.fromObject}, function (process) {
                        if (process.type === 'import-zip') {
                            if (angular.element('.zip-extraction-queue').length === 0) {
                                $alert({title: $translate.instant('PROCESSES.IMPORT_ZIP'), content: $translate.instant('PROCESSES.JUST_COMPLETED', {name: process.name}), type: 'success'});
                            }
                        }
                        //else {
            ...
            //
            //function init() {
            //    console.log('INIT RUNTIME SERVICE');
            //    User.sessionInitialized().then(function () {
            //        if (User.isAuthenticated()) {
            //            getProcesses();
            //            getTasks();
